...
source("C:/Users/mdodrill/Desktop/FB_Git/Drift/FB_Drift_Functions_Git_V3.R", chdir = F)
...
# drift_specimen = 'DriftSpecimen_3_5_18.csv'
# drift_sample   = 'DriftSample_3_5_18.csv'

drift_specimen = 'DriftSpecimen_08_20_2018.csv'
drift_sample   = 'DriftSample_08_20_2018.csv'
...
data = drift_summary(c(2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018), sp_num)
...
# Jeff: the lf.q file has 2017 data which we were using to match above, then
# dropping here, but now, I've removed this to avoid dropping 2018, however,
# this will include some data (na's for no matching q) in this version of the
# model fitting
# counts.data3 = counts.data2[which(!is.na(counts.data2$q)),]
counts.data3 = counts.data2
...
for(i in 1:length(sp.)){
  ...
  fm_1 <- glmmTMB(count.sum ~ 0 + year + (1|month) + (1|DriftRM) + offset(log(Volume_Sampled_M.3)),
...
#-----------------------------------------------------------------------------#
tmp = ifelse(model.counts$sp == 2, "Mud Snails", 
             ifelse(model.counts$sp == 4, "Gammarus", 
                    ifelse(model.counts$sp == 7, "Black Flies", "Midges")))

model.counts$sp2 = factor(tmp, levels = c("Mud Snails", "Gammarus", "Black Flies", "Midges"))


p = ggplot(model.counts, aes(x = year2, y = est)) +
  geom_point() +  # aes(color = year)
  geom_errorbar(aes(ymax = ul, ymin = ll)) + #, color = year  
  scale_x_continuous(breaks = seq(2007, 2018, 1), labels = substr(seq(2007, 2018, 1), 3, 4)) +
  facet_wrap(~ sp2, scales = "free_y") + 
  labs(title = "Long-Term Drift Monitoring - Lees Ferry",
       y = expression(paste('Count / m'^'3')), x = "")  # (1|yr.mo) or (1|month)
p
...
for(i in 1:length(sp.)){
  ...
  fm_1 <- glmmTMB(count.sum ~ 0 + (1|year) + month + (1|DriftRM) + offset(log(Volume_Sampled_M.3)),
                  data = sp.sub,
                  family = nbinom2)
...
#-----------------------------------------------------------------------------#
tmp = ifelse(model.counts.2$sp == 2, "Mud Snails", 
             ifelse(model.counts.2$sp == 4, "Gammarus", 
                    ifelse(model.counts.2$sp == 7, "Black Flies", "Midges")))

model.counts.2$sp2 = factor(tmp, levels = c("Mud Snails", "Gammarus", "Black Flies", "Midges"))
